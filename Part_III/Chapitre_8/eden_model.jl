...
# ╔═╡ e5501964-8dbd-11eb-09a2-69e7fe63cbda
begin
	using Agents, Random
	...
	using Colors
	using ColorSchemes
	mutable struct Bacteria <: AbstractAgent
	    id::Int
	    pos::Dims{2}
	    alive::Bool
	end
...
# ╔═╡ 2c74c6df-09f7-4b76-a17d-6c8769a0fb7c
md"# Eden model"

# ╔═╡ 11bdea44-8dbe-11eb-23e7-81a63c34a02c
function bacteria_init(; posx = 30, griddims = (30, 30))
	space = GridSpace(griddims; periodic = false, metric = :euclidean)
	model = AgentBasedModel(Union{Bacteria}, space,rng = RandomDevice())
	bacteria = Bacteria(nextid(model),(15,15),true)
	add_agent_pos!(bacteria, model)
	return model
end;

# ╔═╡ e1861870-98a2-11eb-1f98-bbb498706a7d
function n_neighbors(agent, model) 
    c = 0
...
# ╔═╡ 5a2b2558-8dbe-11eb-1c42-df4936f50e48
function model_step!(model)
	chosen_sites = []
	for agent in allagents(model)
		if isa(agent,Bacteria)
			n = n_neighbors(agent,model)
			println(n)
			if n == 4
				agent.alive == false
			elseif agent.alive == true
				push!(chosen_sites, agent)
			end
		end
	end
	chosen_site = rand(model.rng, chosen_sites)
	new_positions = [n for n in nearby_positions(chosen_site, model) if isempty(n,model)]
	new_position = rand(model.rng, new_positions)
	if rand(model.rng) < 1.1
		b = Bacteria(nextid(model), new_position,true)
		add_agent_pos!(b, model)
	end
end;

# ╔═╡ f46622b8-985d-11eb-077f-83ef9ac0d1ea
agent_step!(agent::Bacteria, model) = nothing;

# ╔═╡ 866808d2-8dbe-11eb-16ec-25783bc9bc58
begin
	shape(a::Bacteria) = :circle
	size(a::Bacteria) = 22
	color(a::Bacteria) = :green
end;

# ╔═╡ 984e8564-8dbe-11eb-3c7e-35d306c72c60
begin
	model = bacteria_init()
	abm_video(
			"eden.mp4",
...
git-tree-sha1 = "1a96e23c4e34219d338897d4fece7ee0bfbc27a0"
...
git-tree-sha1 = "fcfc4089402d8ff3f108dddf943a9d1449385ac8"
